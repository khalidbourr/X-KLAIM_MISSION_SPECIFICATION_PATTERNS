...
public class Fair_Patrolling extends KlavaProcess {
  ...
  public Fair_Patrolling(final String robotId, final List<Location> locations, final Integer iterations) {
  ...
  public void executeProcess() {
    ...
    while ((count < (this.iterations).intValue())) {
      {
        Fair_Visit _fair_Visit = new Fair_Visit(this.robotId, this.locations);
        eval(_fair_Visit, this.self);
        in(new Tuple(new Object[] {RobotConstants.FAIR_VISIT_COMPLETED}), this.self);
        count = (count + 1);
      }
    }
    out(new Tuple(new Object[] {RobotConstants.FAIR_PATROLLING_COMPLETED}), this.self);
  }
}
